...
int main(int argc, char *argv[])
{
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--host") == 0 || strcmp(argv[i], "-h") == 0) {
            ...
            printf("  %s --host 192.168.1.100 --user admin --password secret\n", argv[0]);
            exit(0);
        }
    }
    ...
    if (hostname) {
        ...
        [invocation setSelector:@selector(connectFromCommandLine:username:password:)];
        [invocation setArgument:&hostname atIndex:2];
        [invocation setArgument:&username atIndex:3];
        [invocation setArgument:&password atIndex:4];
