...
int main(int argc, const char *argv[]) {
    @autoreleasepool {
        ...
        NSDebugLLog(@"gwcomp", @"=== Real daemon message analysis ===");
        NSDebugLLog(@"gwcomp", @"Total length: %lu bytes", sizeof(realBytes));
        
        // Parse fixed header
        NSDebugLLog(@"gwcomp", @"\n--- Fixed Header (16 bytes) ---");
        NSDebugLLog(@"gwcomp", @"Endian: 0x%02x", realBytes[0]);
        NSDebugLLog(@"gwcomp", @"Type: 0x%02x", realBytes[1]);
        NSDebugLLog(@"gwcomp", @"Flags: 0x%02x", realBytes[2]);
        NSDebugLLog(@"gwcomp", @"Version: 0x%02x", realBytes[3]);
        
        uint32_t bodyLength = *(uint32_t*)(realBytes + 4);
        uint32_t serial = *(uint32_t*)(realBytes + 8);
        uint32_t fieldsLength = *(uint32_t*)(realBytes + 12);
        
        NSDebugLLog(@"gwcomp", @"Body length: %u", bodyLength);
        NSDebugLLog(@"gwcomp", @"Serial: %u", serial);
        NSDebugLLog(@"gwcomp", @"Fields length: %u", fieldsLength);
        ...
        NSDebugLLog(@"gwcomp", @"Fields start at: %ld, end at: %ld", fieldsStart - realBytes, fieldsEnd - realBytes);
        NSDebugLLog(@"gwcomp", @"Body starts at: %ld", bodyStart - realBytes);
        ...
        while (pos < fieldsEnd) {
            if (*pos == 0) {
                NSDebugLLog(@"gwcomp", @"Padding byte at offset %ld", pos - realBytes);
                pos++;
                continue;
            }
            ...
            NSDebugLLog(@"gwcomp", @"  Field code: %u", fieldCode);
            
            uint8_t sigLen = *pos++;
            NSDebugLLog(@"gwcomp", @"  Signature length: %u", sigLen);
            
            NSDebugLLog(@"gwcomp", @"  Signature: ");
            ...
            NSDebugLLog(@"gwcomp", @"  Null terminator: 0x%02x", nullTerm);
            
            // Align to 4 for the value
            while ((pos - realBytes) % 4 != 0) {
                NSDebugLLog(@"gwcomp", @"  Value alignment padding at %ld: 0x%02x", pos - realBytes, *pos);
                pos++;
            }
            ...
            if (fieldCode == 6 || fieldCode == 7) { // String fields
                ...
                NSDebugLLog(@"gwcomp", @"  String value: ");
                ...
                NSDebugLLog(@"gwcomp", @"  Signature string length: %u", sigStrLen);
                NSDebugLLog(@"gwcomp", @"  Signature string: ");
            ...
            while (pos < fieldsEnd && (pos - fieldsStart) % 8 != 0) {
                NSDebugLLog(@"gwcomp", @"  Struct alignment padding at %ld: 0x%02x", pos - realBytes, *pos);
                pos++;
            }
        }
        ...
        for (uint32_t i = 0; i < bodyLength && (bodyStart + i - realBytes) < sizeof(realBytes); i++) {
            NSDebugLLog(@"gwcomp", @"Body[%u] = 0x%02x ('%c')", i, bodyStart[i], bodyStart[i] >= 32 && bodyStart[i] < 127 ? bodyStart[i] : '.');
        }
    }
    return 0;
}
