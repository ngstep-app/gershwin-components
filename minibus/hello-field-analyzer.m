...
int main(int argc, const char * argv[]) {
    @autoreleasepool {
        ...
        NSDebugLLog(@"gwcomp", @"MiniBus Hello message length: %lu", (unsigned long)[messageData length]);
        ...
        if ([messageData length] >= 16) {
            ...
            NSDebugLLog(@"gwcomp", @"Header fields length: 0x%02x (%u bytes)", headerFieldsLength, headerFieldsLength);
            
            // Expected from working: 0x6e = 110 bytes
            // MiniBus produces: 0x70 = 112 bytes
            NSDebugLLog(@"gwcomp", @"Difference from working (110 bytes): %d bytes", (int)headerFieldsLength - 110);
            
            // Dump the header fields section
            NSDebugLLog(@"gwcomp", @"Header fields hex dump:");
            for (uint32_t i = 16; i < 16 + headerFieldsLength && i < [messageData length]; i += 16) {
                ...
                NSDebugLLog(@"gwcomp", @"%04x: %-48s %@", i, [hexLine UTF8String], asciiLine);
            }
            ...
            while (pos < fieldEnd && pos < [messageData length]) {
                ...
                while (pos % 8 != 0 && pos < fieldEnd) {
                    NSDebugLLog(@"gwcomp", @"Padding byte at 0x%04x: 0x%02x", pos, bytes[pos]);
                    pos++;
                }
                ...
                NSDebugLLog(@"gwcomp", @"Field %d at 0x%04x: code=%d, sigLen=%d", fieldNum, fieldStart, fieldCode, sigLen);
                
                if (sigLen > 0 && pos + sigLen < fieldEnd) {
                    // Read signature
                    char signature = bytes[pos];
                    NSDebugLLog(@"gwcomp", @"  Signature: '%c'", signature);
                    pos += sigLen + 1; // skip signature + null
                }
                
                // Align to 4-byte boundary for value
                while (pos % 4 != 0 && pos < fieldEnd) {
                    NSDebugLLog(@"gwcomp", @"  Value alignment padding at 0x%04x: 0x%02x", pos, bytes[pos]);
                    pos++;
                }
                ...
                NSDebugLLog(@"gwcomp", @"  Field ends at 0x%04x", pos);
            }
            
            NSDebugLLog(@"gwcomp", @"\nTotal fields parsed: %d", fieldNum);
            NSDebugLLog(@"gwcomp", @"Final position: 0x%04x", pos);
            NSDebugLLog(@"gwcomp", @"Header fields should end at: 0x%04x", fieldEnd);
        }
        ...
        NSDebugLLog(@"gwcomp", @"Working Hello header fields length: 0x6e (%d bytes)", 0x6e);
        NSDebugLLog(@"gwcomp", @"Header fields hex dump (working):");
        ...
        NSDebugLLog(@"gwcomp", @"Comparing header fields (MiniBus vs Working):");
        ...
        for (uint32_t i = 0; i < maxLen; i++) {
            ...
            if (miniByte != workByte) {
                NSDebugLLog(@"gwcomp", @"DIFF at offset %d: MiniBus=0x%02x Working=0x%02x", i, miniByte, workByte);
            }
        }
    }
