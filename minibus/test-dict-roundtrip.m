...
int main(int argc, char *argv[]) {
    @autoreleasepool {
        NSDebugLLog(@"gwcomp", @"Testing dictionary roundtrip...");
        ...
        if (parsed) {
            ...
            if ([parsed.arguments count] == 1 && [[parsed.arguments objectAtIndex:0] isKindOfClass:[NSArray class]]) {
                ...
                NSDebugLLog(@"gwcomp", @"✓ Parsed dictionary array with %lu entries", [parsedDictArray count]);
                
                for (NSUInteger i = 0; i < [parsedDictArray count]; i++) {
                    id entry = [parsedDictArray objectAtIndex:i];
                    NSDebugLLog(@"gwcomp", @"  Entry %lu: %@ (class: %@)", i, entry, [entry class]);
                }
            } else {
                NSDebugLLog(@"gwcomp", @"✗ Parsed arguments structure doesn't match expected");
            }
        } else {
